...
// 페이지 스크롤에 따른 요소 제어 일정량 이상 스크롤하면 보이지 않도록 수정
const badgeEl = document.querySelector('header .badges');
const toTopEl = document.querySelector('#to-top');

window.addEventListener('scroll', function () {
    console.log(window.scrollY);
    if (this.window.scrollY > 500) {    //화면의 스크롤 위치가 500보다 크다면 배지가 보이지 않는다
        // Badge 요소 숨기기
        gsap.to(badgeEl, .6, {
            opacity: 0,
            display: 'none'
...
gsap.to('.floating1', 1.5, {
    ...
    youo: true, // 한번 재생된 애니메이션을 다시 뒤로 재생
